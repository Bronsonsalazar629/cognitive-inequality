...
def test_run_ses_cognition_discovery():
    ...
    result = discover_ses_cognition_paths(df, dataset_name='test', mediators=['has_insurance'])
...
def test_pc_with_mediators():
    ...
    assert adj.loc['ses_index', 'has_insurance'] != 0 or adj.loc['has_insurance', 'ses_index'] != 0
...
def test_pc_without_llm():
    ...
    result = discover_ses_cognition_paths(df, dataset_name='test', mediators=['mediator'], use_llm=False)
    assert 'adjacency' in result
...
def test_pc_with_llm_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        'src.analysis.pc_algorithm.refine_graph', mock_refine
    )

    df = _make_synthetic_dag(n=200)
    df = df.rename(columns={'X': 'ses_index', 'Y': 'cognitive_score', 'M': 'mediator'})

    result = discover_ses_cognition_paths(df, dataset_name='test', mediators=['mediator'], use_llm=True)
    assert len(calls) == 1
